...
<template>
  <header class="fixed left-0 right-0 top-2 z-999 mx-auto w-full flex select-none items-center justify-center line-height-none all:transition-200">
    <nav class="liquid flex items-center justify-start rd-full bg-[rgba(255,255,255,0.05)] p-0.5 dark:bg-[rgba(255,255,255,0.09)]">
      ...
      <ul class="m-0 flex select-none list-none items-center justify-center p-0 text-gray-500 all:transition-100">
        <li v-for="link in links" :key="link.id" class="m-0 flex flex-shrink-0 cursor-pointer items-center justify-center p-0">
          <router-link
            ...
            :style="{ backgroundColor: route.path === link.link ? 'rgba(255,255,255,0.08)' : 'transparent' }"
          >
            {{ link.name }}
          </router-link>
        </li>
    ...
    <ul
      class="liquid m-l-2 h-auto w-9 flex list-none items-center justify-start overflow-hidden rd-full bg-[rgba(255,255,255,0.05)] p-0.5 transition-width duration-200 ease xs-w-auto dark:bg-[rgba(255,255,255,0.09)]"
      :style="{ width: route.path === '/search' ? '2.22rem' : 'auto' }"
    >
      <li class="m-0 cursor-pointer p-0">
        <router-link
          to="/search"
          class="mr-[2px] h-9 w-9 flex flex-justify-center flex-items-center rd-full text-black/75 dark:text-white"
        >
