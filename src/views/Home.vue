  ...
  <main v-if="posts?.length > 0" class="line-height-none select-none">
    <article v-for="post in posts" :key="post.id" class="group mb-12 cursor-pointer">
      <div v-if="post.milestone?.title" class="flex m-y-3">
        <span class="p-x-2 p-y-1 bg-gray-100 font-size-3.4 border-rd text-gray-400 group-hover:text-gray-500 transition-1">{{ post.milestone.title }}</span>
      </div>
      <h2 class="m-0 font-size-8 text-gray-600 font-bold group-hover:text-gray-800 transition-1">
        {{ post.title }}
      </h2>
      <p class="m-y-2 font-size-4 text-gray-400 line-height-normal group-hover:text-gray-500 transition-1 line-2">
        {{ post.summary }}
      </p>
      <p class="m-y-4 font-size-3.4 text-gray-400 group-hover:text-gray-500 transition-1">
    ...
    <div class="mb-12 flex items-center gap-6 text-gray-400 font-size-4 cursor-pointer">
      <span v-show="showMore" class="flex items-center gap-1 hover:text-gray-600 transition-1" @click="loadMore">加载更多<i class="fa-solid fa-chevron-right font-size-3.4" /></span>
    </div>
  </main>
